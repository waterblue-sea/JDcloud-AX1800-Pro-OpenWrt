...
find target/linux/qualcommax -name "config-*" -o -name "config-default" | while read -r kernel_cfg; do
    
    echo "CONFIG_SKB_EXTENSIONS=y" >> "$kernel_cfg"
    echo "CONFIG_NET_TC_SKB_EXT=y" >> "$kernel_cfg"
    echo "CONFIG_NET_RX_BUSY_POLL=y" >> "$kernel_cfg"
    
    echo "# CONFIG_NF_CONNTRACK_DSCPREMARK_EXT is not set" >> "$kernel_cfg"
    echo "# CONFIG_NF_CONNTRACK_CHAIN_EVENTS is not set" >> "$kernel_cfg"
    echo "# CONFIG_NF_CONNTRACK_RTCACHE is not set" >> "$kernel_cfg"
    echo "# CONFIG_SKB_RECYCLE is not set" >> "$kernel_cfg"
    echo "# CONFIG_SKB_RECYCLE_MAX_PREALLOC is not set" >> "$kernel_cfg"
done
